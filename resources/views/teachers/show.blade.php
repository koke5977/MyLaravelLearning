                    ...
                    <section class="text-gray-600 body-font relative">
                        <div class="container px-5 py-5 mx-auto">
                            <div class="flex flex-wrap -m-2">
                                ...
                                <!-- 削除フォーム -->
                                <form id="delete_{{ $teacher->id }}" method="post" action="{{ route('teachers.destroy', $teacher) }}">
                                    @csrf
                                    @method('DELETE')
                                    <div class="p-2 w-full">
                                        <button type="button" data-id="{{ $teacher->id }}" onclick="deletePost(this)" class="flex mx-auto text-white bg-pink-500 border-0 py-2 px-8 focus:outline-none hover:bg-pink-600 rounded text-lg">削除する</button>
                                    </div>
                                </form>
                            </div>
                        </div>
